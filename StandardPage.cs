using System.ComponentModel.DataAnnotations;

[ContentType(
    DisplayName = "Standard page",
    GUID = "934E7266-FB8C-4DEA-B033-3B4E6AE6CBCF",
    Description = "The standard page.",
    AvailableInEditMode = true)]
[ImageUrl("~/styles/images/page_type.png")]
public class StandardPage : PageData
{
    [CultureSpecific]
    [Display(
        Name = "Title",
        Description = "Title for the page",
        GroupName = SystemTabNames.Content,
        Order = 1)]
    public virtual string Title { get; set; }
    
    public virtual IList<string> ListOfStrings { get; set; } // Works
    
    public virtual IList<int> ListOfIntegers { get; set; } // Works
    
    public virtual IList<ContactBlock> ContactsList { get; set; } // Doesn't work
}

[ContentType(AvailableInEditMode = false, GUID = "38d57768-e09e-4da9-90df-54c73c61b270")]
public class ContactBlock : BlockData
{
    public virtual string TestString { get; set; }

    public virtual int TestInt { get; set; }
}